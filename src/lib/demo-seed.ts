// Shared demo seeder. One source of truth for the canonical demo state,
// callable from both the CLI (prisma/seed.ts → npm run db:seed) and the
// super-admin's "Reset demo data" button (/api/admin/reset-demo).
//
// It WIPES every table, then recreates: 3 NGOs (Helping Hands, Care First,
// ReliefNet), 3 NGO admins + 6 volunteers (password relief123), 8 skills,
// and two multi-NGO incidents with timelines + linked needs.
import {
  PrismaClient,
  UserRole,
  VolunteerStatus,
...
export const DEFAULT_PASSWORD = "relief123";
